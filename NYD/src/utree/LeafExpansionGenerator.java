...
public class LeafExpansionGenerator {
	...
	private int maxDepth, maxHistory, currentDepth;
	...
	public LeafExpansionGenerator(Vector conjunctionOfParametersAtLeaf,
			int maxDepth, int maxHistory, Agent agent,
			boolean subDividePerceptions) {
	...
	private ParameterSelected nextElement(ParameterSelected lastElement) {
		...
		do {
			...
			d++;
			if (d >= numberPerceptualDimentions()) {
			...
			newElement = new ParameterSelected(h, d, subDividePerceptions);
