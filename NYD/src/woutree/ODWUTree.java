...
import core.Action;
import core.ActionSelection;
import core.Agent;
import core.Args;
import core.ArithmeticFloat;
import core.ArithmeticInteger;
import core.ReallyBasicStateActionLearner;
import core.State;
import core.StateActionPair;
import core.StateActionValuesInitiallyZero;
import core.UsageArgumentVector;
import core.WorldSetUpException;
 ...
 * 1-12th August 2005
 * 
 * - added flag to allow selection of actions compared for KS test. Select
 * between 'all' (previous default) and just 'policy' actions the latter as used
 * in McCallums code.
 * 
 ...
 * 
 * - added option to prefer the shortest fringe extension which produces a utile
 * distinction for an existing leaf. Secondary criteria to prefer max KS
 ...
 * 14th August 2005
 * 
 * - removed preference option; always prefer shortest with KS d-statisitic as
 * secondary preference. This speeds things up and McCallum's code appears to do
 * the same.
 *
 *
 *
 * 
 * REMINDER: probability measure return by KS is the probability that both data
 * sets come from the *same* distribution.
 * 
 * KS probability = 1 -> same distribution. KS probability = 0 -> strongly
 * different distributions.
 * 
 * therefore the smaller the value used for 'KSprob' the more distinct the
 * distributions need to be.
 *
 * default value used by McCallum's code KSprob=0.2 value used in SAB'96 & PhD
 * for NY driving problem = 0.0001
 * 
 * 
 * (default value used by McCallum for minInstances4KsTest = 10)
 */
@SuppressWarnings({"unchecked","rawtypes"})
public class ODWUTree extends ReallyBasicStateActionLearner {

	protected String kSteps, minFringeDepth, maxFringeDepth, maxHistory,max_order_observation;
	...
	private Hashtable<String,Integer> observationRecord = new Hashtable<>();

	// constructor
	public ODWUTree(Agent agent, String args) throws Exception {
		...
		if (!(saInitValue instanceof StateActionValuesInitiallyZero))
			throw new WorldSetUpException("UTree only works with \"initSAvs=zeros\".");
		...
		max_order_observation = Args.getArgument(args, "max_order_observation=");
		String yesNo = Args.getArgument(args, "subPercepts=");

		minInstances4KsTest = Integer.parseInt(Args.getArgument(args, "minIs="));
		...
		if (subDividePerceptions && !agent.observationDimensionsFixed())
			throw new WorldSetUpException("Can't subdivide perceptions for specified agent.");

		if (!actionsToCompare.equals("all") && !actionsToCompare.equals("policy"))
			throw new Exception(usageString());
		...
		evalFormula(max_order_observation);
	}

	// list arguments, e.g. kSteps, alpha, gamma, etc., and current values
	public String listActnLearnArguments() {
		return super.listActnLearnArguments() + "\tkSteps\tminFringe\tmaxFringe\tmaxHistory\tKSprob";
	}

	public String currentValueActnLearnArgs() {
		return (super.currentValueActnLearnArgs() + "\t" + String.valueOf(evalFormula(kSteps)) + "\t" + String.valueOf(evalFormula(minFringeDepth)) + "\t\t" + String.valueOf(evalFormula(maxFringeDepth)) + "\t\t" + String.valueOf(evalFormula(maxHistory)) + "\t\t" + String.valueOf(evalFormula(ksProbability)));
	}
	...
	// deal with
	public Object clone() {
		ODWUTree aClone = (ODWUTree) super.clone();
	...
	public void mainLoop2() {
		...
		State o = agent.getState();
		/*System.out.println("----------------------------------");
		for (Object object : o.observations) {
...
//		System.out.println(tee);
	...
	public void exploreLoop(){
		State o = agent.getState();
		Action a = selectAction(o, true);
		Float r = executeAction(a, o);
		
		int action_occur_num = observationRecord.get(a.toString());
		observationRecord.put(a.toString(), action_occur_num+1);
		for (Object string : o.observations) {
			int occur_num = observationRecord.get(string);
			observationRecord.put((String) string, occur_num+1);
		}
	}
...
//		System.out.println(a);
		...
		// transition values
		((UTreeLeafData) nodeS.data).addInstanceIndex(instanceChain.indexOf(tee)); // add the current instance
		...
		{ // (use block to limit scope of variables)
			...
			if (transitionCounts.containsKey(nodeSdash)) {
				((ArithmeticInteger) transitionCounts.get(nodeSdash)).increment();
			} else {
		...
		for (Enumeration e = allLeaves.elements(); e.hasMoreElements();) {
			Hashtable actionValues = ((UTreeLeafData) ((TreeNode) e.nextElement()).data).actionValues;
			for (Enumeration f = actionValues.elements(); f.hasMoreElements();) {
				...
				if (N == 0) {
					System.err.print("Division by zero (N=0) in UTree, mainLoop(), " + "one sweep of dynamic programming.");
					System.exit(1);
				}
				float R = ((ArithmeticFloat) values.elementAt(2)).value / ((float) N);
				float sumPrU = 0;
				Hashtable transitionCounts = (Hashtable) values.elementAt(3);
				for (Enumeration g = transitionCounts.keys(); g.hasMoreElements();) {
					...
					Double U = null;
					// get max Q(s',a') --> U(s')
					for (Enumeration h = aDashValues.elements(); h.hasMoreElements();) {
						Double uTemp = (Double) ((Vector) h.nextElement()).firstElement();
					...
					if (U == null)
						U = new Double(0.0);
					sumPrU += U.floatValue() * (float) ((ArithmeticInteger) transitionCounts.get(nodeSdash2)).value / ((float) N);
				}
		...
		if (actionSteps % evalFormula(kSteps) == 0) {
			...
			for (int e = 0; e < allLeaves.size(); e++) {
				...
				if (indexOfInstances.size() >= minInstances4KsTest) {
					...
					Hashtable observationReward = new Hashtable();
					for (Iterator iterator = observationRecord.keySet().iterator(); iterator.hasNext();) {
						String dimension_key = (String) iterator.next();
						observationReward.put(dimension_key, 0f);
					}
					int reward_count=0;
					for (Enumeration f = indexOfInstances.elements(); f.hasMoreElements();++reward_count) {
						Instance instance = instanceChain.getInstance((Integer) f.nextElement());
						Float reward = instance.reward();
						
						/*for (int i = 0; i <= agent.numberOfObservationDimensions(); i++) {
							System.out.print(instance.dimension(i, subDividePerceptions)+" ");
						}*/
						
						for (int i = 0; i <= agent.numberOfObservationDimensions(); i++) {
							String dimension_key = instance.dimension(i, subDividePerceptions).toString();
							Float current_reward = (Float) observationReward.get(dimension_key);
							observationReward.put(dimension_key, (current_reward*reward_count+reward)/(reward_count+1));
						}
						...
						Double Qvalue = new Double(instance.reward().floatValue() + evalFormula(gamma) * U.doubleValue());
						instanceListQvalues.put(instance, Qvalue);
					}

					// Total possible splits = number of dimensions * max.history index.
					...
					for (LeafExpansionGenerator f = new LeafExpansionGenerator(
							...
							(int) evalFormula(max_order_observation), 
							agent, 
							subDividePerceptions,
							observationRecord,
							observationReward); 
							f.hasNextExpansion() && 
							(
									strongestFringe == null || f.nextExpansionLength() <= strongestFringeDepth.intValue());
							) 
					{
						...
						if (expansion.size() >= evalFormula(minFringeDepth)) {
							...
							for (Enumeration g = expansion.elements(); g.hasMoreElements();) {

								ParameterSelected parameter = (ParameterSelected) g.nextElement();

								for (Enumeration h = fringe.allLeaves().elements(); h.hasMoreElements();) {
									TreeNode fringeLeaf = (TreeNode) h.nextElement();
									...
									for (Enumeration i = fringeLeafIndexOfInstances.elements(); i.hasMoreElements();) {
										int instanceIndex = ((Integer) i.nextElement()).intValue();
										Instance instance = instanceChain.getInstance(instanceIndex);
										...
										// in the fringe.
										Object parameterValue = parameter.getParameterValue(instanceIndex - 1, instanceChain);
										TreeNode subFringeLeaf;
										if (fringeLeaf.branchExistsForParameterValue(parameterValue))
											subFringeLeaf = fringeLeaf.getSubTreeNode(parameterValue);
										else {
											subFringeLeaf = new TreeNode(new UTreeLeafData());
											fringeLeaf.addSubTreeNode(parameterValue, subFringeLeaf);
										}
										// add instance to sub fringe leaf
										((UTreeLeafData) subFringeLeaf.data).addInstanceIndex(instanceIndex);
										// update or populate actionValue table
										Hashtable subFringeActionValues = ((UTreeLeafData) subFringeLeaf.data).actionValues;
										Vector values;
										if (subFringeActionValues.containsKey(instance.actionName())) {
											values = (Vector) subFringeActionValues.get(instance.actionName());
										} else {
											// if doesn't exist create new
											// Vector
											values = new Vector();
											// and add to actionValues
											subFringeActionValues.put(instance.actionName(), values);
											// initiate Q value
											values.add(new Double(0.0));
											...
											values.add(new ArithmeticFloat(0.0f));
											// initiate table of transition
											// probabilities
											values.add(new Hashtable());
										}
										Double Q = ((Double) values.elementAt(0));
										ArithmeticInteger N = ((ArithmeticInteger) values.elementAt(1));
										Q = new Double((Q.doubleValue() * N.value + ((Double) instanceListQvalues.get(instance)).doubleValue()) / (N.value + 1));
										values.set(0, Q); // update Q value
										N.increment(); // increment N
										// increment sum of r
										((ArithmeticFloat) values.elementAt(2)).plus(instance.reward());
							...
							Vector leafPolicyActions = leafValue.getMaxQActionNames();
							...
							for (Enumeration g = leafValue.actionValues.keys(); g.hasMoreElements();) {
								String actionName = (String) g.nextElement();
								Vector leafQvaluesGivenAction = new Vector();
								for (Enumeration h = indexOfInstances.elements(); h.hasMoreElements();) {
									Instance instance = instanceChain.getInstance((Integer) h.nextElement());
									if (instance.actionName().equals(actionName)) {
										leafQvaluesGivenAction.add(instanceListQvalues.get(instance));
									}
								}
								...
								for (Enumeration h = fringe.allLeaves().elements(); h.hasMoreElements();) {
									...
									Vector fringeLeafPolicyActions = ((UTreeLeafData) fringeLeaf.data).getMaxQActionNames();
									...
									if (actionsToCompare.equals("all") || leafPolicyActions.contains(actionName) || fringeLeafPolicyActions.contains(actionName)) {

										Vector fringeLeafQvaluesGivenAction = new Vector();
										// get Q values for relevant fringe
										// instances
										for (Enumeration i = ((UTreeLeafData) fringeLeaf.data).indexOfInstances.elements(); i.hasMoreElements();) {
											Instance instance = instanceChain.getInstance((Integer) i.nextElement());
											if (instance.actionName().equals(actionName)) {
												fringeLeafQvaluesGivenAction.add(instanceListQvalues.get(instance));
											}
										}
										...
										if (fringeLeafQvaluesGivenAction.size() >= minInstances4KsTest) {
											...
											// do KS comparison MannWhitneyU
											double[] results = KolmogorovSmirnov.testTwoDataSets(leafQvaluesGivenAction, fringeLeafQvaluesGivenAction);
//											double[] results = MannWhitneyU.testTwoDataSets(leafQvaluesGivenAction, fringeLeafQvaluesGivenAction);
											// If probability P is small (i.e. <
											// ksProb) then a utile fringe
											// found.
											// If its also the strongest found
											// then make a note of it.
											if (results[1] < evalFormula(ksProbability) && (strongestFringe == null || (expansion.size() <= strongestFringeDepth.intValue() && results[0] > strongestKSd.doubleValue()))) {
												strongestFringe = fringe;
												strongestKSd = new Double(results[0]);
												strongestFringeDepth = new Integer(expansion.size());
											}
										}
									}
			...
			if (fixTransitions) {
				for (Iterator i = allLeaves.iterator(); i.hasNext();) {
					...
					for (Enumeration j = actionValues.keys(); j.hasMoreElements();) {
						String actionName = (String) j.nextElement();
						((Vector) actionValues.get(actionName)).set(3, new Hashtable());
					}
					// find out where the instances now end up and count
					// transitions
					for (Iterator j = indexOfInstances.iterator(); j.hasNext();) {
						Instance instance = instanceChain.getInstance((Integer) j.next());
						...
						// some of the parameter values of instances in this
						// leaf
						// may not have been seen before.
						Vector values = (Vector) actionValues.get(instance.actionName());
						Hashtable transitionCounts = (Hashtable) values.elementAt(3);
						if (transitionCounts.containsKey(nodeS2)) {
							((ArithmeticInteger) transitionCounts.get(nodeS2)).increment();
						} else {
							transitionCounts.put(nodeS2, new ArithmeticInteger(1));
						}
					}
				}
			}
	...
	private TreeNode getNodeS(Instance instance, boolean addLeavesToTree) {
		...
		while (!nodeS.isLeaf()) {
			...
			// stored.
			Object parameterValue = selector.getParameterValue(instance, instanceChain);
			...
			if (newNodeS == null) {
				if (addLeavesToTree) {
					...
					// * just need to create an empty node; instances that would be stored *
					// * in this node are the instances relating to the next state s(T+1), not *
					// * this instance which is stored else where. *
					/*((UTreeLeafData) newNodeS.data).addInstanceIndex(instanceChain.indexOf(instance));
					Hashtable newActionValues = ((UTreeLeafData) newNodeS.data).actionValues;
					Vector values = new Vector();
					newActionValues.put(instance.actionName(), values);
					values.add(new Double(instance.reward().doubleValue())); // initial Q value equal to reward
					values.add(new ArithmeticInteger(1)); // init. number of entries (N)
					values.add(new ArithmeticFloat(instance.reward())); // init. sum of rewards (sum r)
					values.add(new Hashtable()); // init. table of transition  probabilities
*/				} else {
					System.err.println("Tree Node: " + nodeS + "/nParameter Value: " + parameterValue + " not seen before.\nNo permission to add new leaf.");
	...
	protected String knowledgeFileNameParameters() {
		return ("k" + kSteps + " fringe" + minFringeDepth + "-" + maxFringeDepth + " maxH" + maxHistory + " KSP" + ksProbability + (subDividePerceptions ? " sub " : " whole ") + actionsToCompare + " " + super.knowledgeFileNameParameters());
	}
	...
	public void dump_observationRecord(){
		for (Iterator iterator = observationRecord.keySet().iterator(); iterator.hasNext();) {
			String dimension_key = (String) iterator.next();
			int occur_num = observationRecord.get(dimension_key);
			System.out.println(dimension_key+":"+occur_num);
		}
	}
	
	public boolean canStopHeuristicExploration(){
		for (Iterator iterator = observationRecord.keySet().iterator(); iterator.hasNext();) {
			String dimension_key = (String) iterator.next();
			int occur_num = observationRecord.get(dimension_key);
			if(occur_num<100)
	...
	public void clearObservationRecord(){
		Hashtable all_percept_action = ((TruckAgent) agent).getAllPerceptAndAction();
		for (Iterator iterator = all_percept_action.keySet().iterator(); iterator.hasNext();) {
			String dimension_key = (String) iterator.next();
			String[] dimension_sub_observation = (String[]) all_percept_action.get(dimension_key);
			for (String observation : dimension_sub_observation) {
				observationRecord.put(observation, 0);
			}
		}
	}
